...
"""
from .nuimages_no_wc_category_image_count import NUIMAGES_NO_WC_CATEGORY_IMAGE_COUNT
...
coco_novel_cat_ids = [1, 2, 3, 4, 5, 6, 7, 9, 16, 17, 18, 19, 20, 21, 44, 62, 63, 64, 67, 72]
COCO_BASE_CATEGORIES = [x for x in COCO_CATEGORIES if x['id'] not in coco_novel_cat_ids and x['isthing']==1]
assert len(COCO_BASE_CATEGORIES)==60, "Check how you are making COCO BASE CATS"
...
def _get_coco_instances_meta_base_only():
    thing_ids = [k["id"] for k in COCO_BASE_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in COCO_BASE_CATEGORIES if k["isthing"] == 1]
    assert len(thing_ids) == 60, len(thing_ids)
    # Mapping from the incontiguous COCO category id to an id in [0, 79]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in COCO_BASE_CATEGORIES if k["isthing"] == 1]
    ret = {
        "thing_dataset_id_to_contiguous_id": thing_dataset_id_to_contiguous_id,
        "thing_classes": thing_classes,
        "thing_colors": thing_colors,
...
def _get_builtin_metadata(dataset_name, dataset_type=None, categories=None, class_image_count=None):
    if dataset_type=='rf':
    ...
    elif dataset_name == 'nuimages_all_cls':
        categories = [
            {'id': 0, 'name': 'car'},
            {'id': 1, 'name': 'truck'},
            {'id': 2, 'name': 'construction_vehicle'},
            {'id': 3, 'name': 'bus'}, 
            {'id': 4, 'name': 'trailer'},
            {'id': 5, 'name': 'emergency'}, 
            ...
            {'id': 12, 'name': 'personal_mobility'}, 
            {'id': 13, 'name': 'wheelchair'},
            {'id': 14, 'name': 'stroller'}, 
            {'id': 15, 'name': 'pushable_pullable'},
            {'id': 16, 'name': 'barrier'}, 
            {'id': 17, 'name': 'traffic_cone'},
            {'id': 18, 'name': 'debris'}
        ]
    ...
    elif dataset_name == 'nuimages_all_cls_no_wc' or 'nuimages_fsod' in dataset_name or 'nuimages_fiod' in dataset_name:  
        categories = [
            {'id': 0, 'name': 'car'},
            {'id': 1, 'name': 'truck'},
            {'id': 2, 'name': 'construction_vehicle'},
        ...
        return {
            "thing_dataset_id_to_contiguous_id": thing_dataset_id_to_contiguous_id,
            "thing_classes": thing_classes,
            "class_image_count": NUIMAGES_NO_WC_CATEGORY_IMAGE_COUNT}
